...
module CalendarInvite =
    ...
    let createCalendarAttachment (event: Event) participantEmail =
        ["BEGIN:VCALENDAR"
         ...
         sprintf "X-ALT-DESC;FMTTYPE=text/html:%s"  (event.Description.ToString())
    ...
    let createMessage (event: Event) participant =
        ...
        ["Hei! 😄"
         ...
         sprintf "Vi gleder oss til å se deg på %s den %s 🎉" event.Location.Unwrap (toReadableString event.StartDate)
