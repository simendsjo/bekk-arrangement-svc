...
module Handlers =
    ...
    let registerForEvent (eventId: Guid, email) =
        result {
            ...
            let createMailForParticipant =
                Service.createNewParticipantMail createCancelUrl event isWaitlisted (EmailAddress config.noReplyEmail)

            let! userId = Auth.getUserId

            let! participantDomainModel =
                writeToDomain (eventId, email) writeModel userId
                |> Task.wrap
                |> ignoreContext
        ...
        result {
            ...
            return
                { attendees =
                      ...
                      |> Seq.map
                          (fun p ->
                              if not userCanGetInformation then
                                  { p with
                                        Email = None
                                        ParticipantAnswers = [] }
                              else
                  ...
                  waitingList =
                      if hasWaitingList then
                          Seq.map domainToView participants.waitingList
                          |> Seq.map
                              (fun p ->
                                  if not userCanGetInformation then
                                      { p with
    ...
    let getWaitinglistSpot (eventId, email) =
        Service.getWaitinglistSpot (Event.Id eventId) (EmailAddress email)


    let exportParticipationsDataForEvent eventId =
        Service.exportParticipationsDataForEvent (Event.Id eventId)
