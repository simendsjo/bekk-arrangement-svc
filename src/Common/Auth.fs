...
module Auth =
    ...
    let isAdmin failure =
        fun next (ctx: HttpContext) ->
            ...
            hasPermission config.permissionsAndClaimsKey
                config.adminPermissionClaim failure next ctx

    let isLoggedIn failure =
        fun next (ctx: HttpContext) ->
            let config = ctx.GetService<AppConfig>()
            hasPermission config.permissionsAndClaimsKey
                config.readPermissionClaim failure next ctx

    let makeSureUserIsAdmin: HttpHandler =
        isAdmin
            (accessDenied "Access Denied, you do not have admin permissions")
